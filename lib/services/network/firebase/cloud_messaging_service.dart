...
class CloudMessagingService {
  ...
  Future<void> registerDevice() async {
    // If the user has disabled FCM (e.g. they rely on UnifiedPush or the background service),
    // skip registration entirely. Nothing is attempted, so a transient Firebase failure can't
    // surface the "Failed to register FCM device!" banner for a transport they aren't using.
    // The server drops this device via its inactivity purge, so FCM stops being a delivery route.
    if (!SettingsSvc.settings.enableFcm.value) {
